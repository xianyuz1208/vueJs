class Vue {
  constructor(options) {
    ...
    this.$mount(options.el)//发现如果有el这个选项,就去挂载就初始化
    }
  }
  initData() {
    let data = this._data
  ...
  $mount(el){
    ...
    //初始化render watcher,这样数据发生改变的时候,才会自动更新视图
    this._watcher = new Watcher(this,()=>{
      /*生成新的vdom*/this._update(this.$options.render.call(this))//获取到当前实例上的所有元素节点
    },()=>{})
  }
  _update(vnode){
    if(this._vnode){
      //如果不是第一次发生改变,就把旧的与新的进行比较与更新
      patch(this._vnode,vnode)
    }else{
      //第一次挂载vue实例,就传一个真实的dom上去
      patch(this.$el,vnode)
    }
    //旧的是从上一次新的vnode中来,所以需要存起来
    this._vnode = vnode
  }
  $watch(key,cb){
